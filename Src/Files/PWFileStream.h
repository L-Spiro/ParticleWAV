 ...
 *
 * Description: A very simple file stream implementation.
 */
...
namespace pw {
	...
	/**
	 * Class CFileStream
	 * \brief A very simple file stream implementation.
	 *
	 * Description: A very simple file stream implementation.
	 */
	class CFileStream : public CStreamBase {
	public :
		CFileStream( CFileBase &_fbFile ) :
			m_fbFile( _fbFile ) {
		}
		...
		bool										Read( _tType &_tValue ) const {
			if PW_LIKELY( Read( reinterpret_cast<uint8_t *>(&_tValue), sizeof( _tValue ) ) == sizeof( _tValue ) ) { return true; }
			return false;
		}
		...
		bool										Write( _tType _tValue ) {
			if PW_LIKELY( Write( reinterpret_cast<const uint8_t *>(&_tValue), sizeof( _tValue ) ) == sizeof( _tValue ) ) { return true; }
			return false;
		}
		...
		virtual size_t								Read( uint8_t * _pui8Dst, size_t _sSize ) const {
			if ( !m_fbFile.Read( _pui8Dst, _sSize ) ) { return 0; }
			return _sSize;
		}
		...
		virtual size_t								Write( const uint8_t * _pui8Src, size_t _sSize ) {
			if ( !m_fbFile.Write( _pui8Src, _sSize ) ) { return 0; }
			return _sSize;
		}
		 ...
		 **/
		virtual uint64_t							MovePointerBy( int64_t _i64Offset ) const { return m_fbFile.MovePointerBy( _i64Offset ); }
		 ...
		 **/
		virtual uint64_t							MovePointerTo( uint64_t _ui64Pos, bool _bFromEnd = false ) const { return m_fbFile.MovePointerTo( _ui64Pos, _bFromEnd ); }
		 ...
		 **/
		virtual size_t								Remaining() const { return static_cast<size_t>(m_fbFile.Size() - m_fbFile.GetPos()); }
		 ...
		 **/
		virtual size_t								Pos() const { return static_cast<size_t>(m_fbFile.GetPos()); }
